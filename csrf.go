...
var CSRFFilter = func(c *revel.Controller, fc []revel.Filter) {
	...
	if !found {
		...
		if len(realToken) != lengthCSRFToken {
			...
			c.Log.Debugf("REVEL_CSRF: Bad token length: found %d, expected %d",
	...
	if unsafeMethod && !IsExempted(r.URL.Path) {
		c.Log.Debugf("REVEL-CSRF: Processing unsafe '%s' method...", r.Method)
		...
		c.Log.Debugf("REVEL-CSRF: Token received from client: '%s'", sentToken)
		...
		c.Log.Debug("REVEL-CSRF: Token successfully checked.")
	}
